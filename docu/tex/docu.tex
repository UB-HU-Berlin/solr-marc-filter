...
Der Ordner \texttt{index\_scripts} in dem sich die Skripte befinden müssen, liegt im SolrMarc Unterverzeichnis \texttt{local\_build}. Die Skripte sind in Java-Code geschrieben und werden im Folgenden erklärt.
...
\begin{longtable}{| l | p{5cm} | p{5cm} | } 
	...
	\texttt{pathResults} & results & Pfad, in denen die Ergebnisse der Filter-Anfragen gespeichert werden (kann in einen beliebigen Pfad geändert werden). \\ \hline
	
	\texttt{resultType} & mrc & Gibt an in welchem Format die Ergebnisse zurückgebeben werden sollen (\texttt{xml} oder \texttt{mrc} möglich) \\ \hline
	
	
	\multicolumn{3}{l}{(Core-spezifisch:)} \\ \hline
	\textbf{Parameter} & \textbf{Beispiel} & \textbf{Erklärung} \\ \hline

	\texttt{updateType} & http & einer von drei möglichen Update Varianten (\texttt{http, ssh, oai}) \\ \hline
	...
	\texttt{updateIntervalInDays} & 6 & Anzahl der Tage nachdem wieder nach neuen Updates gesucht werden soll, seit dem letzten Update, \texttt{6} würde wöchentliche Updates bedeuten, wenn immer zur gleichen Zeit, bspw. Mitternachts nach Updates gesucht wird (\textbf{Hinweis:} da das Herunterladen auch Zeit benötigt, sollte der Parameter immer um eins verringert werden) \\ \hline

	\texttt{updates} & data/swb/updates/ & Pfad in dem die Updates gespeichert werden; sollte nicht verändert werden \\ \hline
	
	\texttt{initialDataFormat} & xml & Formatangabe der Initialdaten (\texttt{xml} oder \texttt{mrc} möglich) \\ \hline
	...
	\texttt{updateFormat} & xml & Format in dem die Updates zur Verfügung gestellt werden (\texttt{xml} oder \texttt{mrc} möglich) \\ \hline
	
	\texttt{urlSolrCore} & http://127.0.0.1:8983/solr/\#/swb & URL unter der der spezifische Core zu erreichen ist \\ \hline
	
	\texttt{check} & 0 & Gibt an, ob der jeweilige Core geupdatet werden soll (\texttt{1}) oder nicht (\texttt{0}). \\ \hline
	
	\texttt{lastUpdate} & 2015-09-03T12:00:00Z & Zeitpunkt des letzten durchgeführten Updates für den jeweiligen Core (dies hat nichts mit dem Datum bzw. der Aktualität des Updates an sich zu tun). Um das letzte heruntergeladene Update zu identifizieren, muss in die letzte Zeile der Datei \texttt{lastUpdates.txt} geschaut werden. \\ \hline
	...
	%\textbf{Parameter} & \textbf{Beispiel} & \textbf{Erklärung} \\ \hline
	%\newpage
	\multicolumn{3}{l}{(HTTP-spezifisch:)} \\ \hline
	
	\texttt{httpUrl} & http://swblod.bsz-bw.de/od/ & URL unter der die Updates ohne Zugangsbeschränkung oder weitere URL-Parameter erreicht und heruntergeladen werden können. \\ \hline
	
	\texttt{httpUrlDeletions} & http://swblod.bsz-bw.de/ loeschungen/ & URL unter der die Löschungen ohne Zugangsbeschränkung oder weitere URL-Parameter erreicht und heruntergeladen werden können (gibt es für den jeweiligen Core keine separate URL für Löschungen, wird hier automatisch \texttt{httpUrl} verwendet). \\ \hline
	
	
	\multicolumn{3}{l}{(OAI-spezifisch:)} \\ \hline
	
	\texttt{oaiUrl} & http://bvbr.bib-bvb.de:8991/ aleph-cgi/oai/oai\_opendata.pl & URL unter der die OAI-Updates ohne Zugangsbeschränkung erreicht werden können (die \texttt{oaiUrl} ist im Grunde auch eine http-Webadresse, hinter der sich aber eine OAI-Schnittstelle befindet) \\ \hline
	
	\texttt{oaiMaxRecordsPerUpdatefile} & 10000 & Da die Updates über die OAI-Schnittstelle geladen werden und unterschiedlich viele Updates pro Tag vorliegen können aber nicht alle Updates in eine einzige Datei geschrieben werden sollte, kann mit diesem Parameter angegeben werden, wieviele Records maximal in einer Update-Datei abgelegt werden (Standardwert \texttt{10000}). \\ \hline
...
\begin{enumerate}
	\item Repository clonen %TODO: (oder letzte veröffentlichte Version herunterladen) Notiz: die letzte Version beinhaltet idR keine ungetesteten Code - stabiler
	...
	\begin{itemize}
		...
		\item[] \texttt{\$ perl -e ”use Apache::Solr; use Archive::Extract; use Config::INI; use Date::Simple, use HTTP::OAI; use LWP::Simple; use LWP::UserAgent; use MARC::Batch; use MARC::Field; use MARC::File; use Net::SCP; use Net::SSH; use Sys::Info; use Text::Unidecode; use Time::Piece; use Try::Tiny; use XML::Simple;" }
...
%	\end{itemize}
	...
	\begin{itemize}
		...
		\item Updates via SCP, HTTP ausführlich getestet
		\item Updates via OAI nur im geringen Umfang getestet
		\item Updates via OAI nur wenn Schnittstellen-Dateiformat XML
	\end{itemize}
